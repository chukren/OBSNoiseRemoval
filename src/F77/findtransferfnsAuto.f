...
c  Pipe in data   findtiltBYtransfer  < findtiltBYtransfer.inp
      ...
      real*4 gausst(maxpts),tdataz(maxpts),tdatahr(maxpts)
      ...
      character*70 foutput, foutput2, foutput3
...
c  foutput2 file for summary output of angle and polynomial fit
...
c  read frequency range for fitting polynomial descriptions of transfer function
      ...
      enddo
c repeat for dummy info for P2Z
      read(*,*) nbandsp
c  read frequency range for fitting polynomial descriptions of transfer function
      do iband = 1,nbandsp
...
c	write(*,*) stddevdataam, stddevdataph, ' amp and phase 
c     1 normalized standard deviations'
c	write(*,*) freqlo(iband), freqhi(iband)
c        write(*,*) change(1),change(2),change(3)
c	write(*,*) change(4),change(5),change(6)
...
cc  end loop over frequency bands
        enddo
      close(unit = 11)
      close(unit = 12)
